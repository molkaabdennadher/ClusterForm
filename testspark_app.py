...
ANSIBLE_CONTROLLER = "192.168.100.14"
NAMENODE = "192.168.100.11"
DATANODE1 = "192.168.100.12"
DATANODE2 = "192.168.100.13"
STANDBY = "192.168.100.15"
...
- name: Déploiement Hadoop via extraction
  ...
  tasks:
    ...
    # 7. Transférer et extraire Zookeeper
    - name: Transférer Zookeeper via SCP
      command: >
        scp -r -i /home/molka/.ssh/id_rsa
        -o StrictHostKeyChecking=no
        -o UserKnownHostsFile=/dev/null
        /etc/apache-zookeeper-3.6.3-bin.tar.gz 
        molka@{{ inventory_hostname }}:/tmp/
      delegate_to: localhost
      when: inventory_hostname in groups['zookeeper']

    - name: Déplacer l'archive Zookeeper
      become: yes
      shell: |
        mv /tmp/apache-zookeeper-3.6.3-bin.tar.gz /etc/
        chown molka:molka /etc/apache-zookeeper-3.6.3-bin.tar.gz
      when: inventory_hostname in groups['zookeeper']

    - name: Extraire Zookeeper
      become: yes
      shell: |
        mkdir -p /etc/zookeeper
        tar xzf /etc/apache-zookeeper-3.6.3-bin.tar.gz -C /etc/zookeeper --strip-components=1 || (echo "Échec extraction Zookeeper" && exit 1)
        chown -R molka:molka /etc/zookeeper
        rm -f /etc/apache-zookeeper-3.6.3-bin.tar.gz
      when: inventory_hostname in groups['zookeeper']
      args:
        executable: /bin/bash

    # 9. Vérifier l'existence du fichier de configuration
    - name: Vérifier l'existence du fichier de configuration
      stat:
        path: /etc/zookeeper/conf/zoo_sample.cfg
      register: ansible_stat

    # 10. Déplacer le fichier de configuration d'exemple
    - name: Déplacer le fichier de configuration d'exemple
      command: mv /etc/zookeeper/conf/zoo_sample.cfg /etc/zookeeper/conf/zoo.cfg
      when: inventory_hostname in groups['zookeeper'] and ansible_stat.stat.exists

    # 11. Ajuster les droits
    - name: Ajuster les droits sur les répertoires Zookeeper
      command: chown -R molka:molka /var/lib/zookeeper /var/log/zookeeper
      when: inventory_hostname in groups['zookeeper']

- name: Déploiement de ZooKeeper
  hosts: zookeeper
  become: yes
  vars:
    # Générer dynamiquement la liste des IDs en fonction du nombre de nœuds
    zookeeper_ids: "{{ range(1, groups['zookeeper'] | length + 1) | list }}"

  tasks:
    # 12. Écrire l'ID de ZooKeeper dans myid
    - name: Configurer le fichier myid pour chaque nœud ZooKeeper
      copy:
        content: "{{ zookeeper_ids[groups['zookeeper'].index(inventory_hostname)] }}"
